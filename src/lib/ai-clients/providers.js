...
var createXaiClient = function () {
  ...
  try {
    ...
    return {
      provider: 'xai',
      model: 'grok-4-fast-reasoning-latest', // Updated to use fast reasoning for architecture
      client: xai,
      isAvailable: true,
    };
  } catch (error) {
...
/**
 * XAI Grok Non-Reasoning client setup
 * Used for: Form Putter agent (fast formatting without complex reasoning)
 */
var createXaiFormatterClient = function () {
  var apiKey = process.env['XAI_API_KEY'];
  if (!apiKey) {
    console.warn('[AI Provider] XAI API key not configured for formatter');
    return null;
  }
  try {
    var xai = createXai({
    ...
    return {
      provider: 'xai-formatter',
      model: 'grok-4-fast-non-reasoning-latest', // Non-reasoning for fast formatting
      client: xai,
      isAvailable: true,
    };
  } catch (error) {
    console.error('[AI Provider] Failed to initialize XAI Formatter client:', error);
    return {
      provider: 'xai-formatter',
...
var createGroqClient = function () {
  ...
  try {
    ...
    return {
      provider: 'groq',
      model: 'openai/gpt-oss-120b', // OpenAI GPT-OSS-120B for high-speed processing
      client: groq,
      isAvailable: true,
    };
  } catch (error) {
...
var AIProvidersManager = /** @class */ (function () {
  ...
  AIProvidersManager.prototype.initialize = function () {
    ...
    try {
      ...
      // Initialize XAI Formatter client (non-reasoning)
      console.log('🔧 [51.5] AI Providers: Initializing XAI Formatter client');
      var xaiFormatterClient = createXaiFormatterClient();
      if (xaiFormatterClient) {
        this.providers.set('xai-formatter', xaiFormatterClient);
        console.log('✅ [52.5] AI Providers: XAI Formatter client initialized');
      } else {
        console.log('⚠️ [53.5] AI Providers: XAI Formatter client not available (missing API key)');
      }
  ...
  AIProvidersManager.prototype.getClientForAgent = function (agentType) {
    var providerMap = {
      architect: 'xai', // XAI Grok-4-fast-reasoning-latest for deep planning
      gatherer: 'groq', // OpenAI GPT-OSS-120B via Groq for fast information processing
      specialist: 'groq', // OpenAI GPT-OSS-120B via Groq for lightning-fast curation
      formatter: 'xai-formatter', // XAI Grok-4-fast-non-reasoning-latest for rapid formatting
    };
  ...
  AIProvidersManager.prototype.getAvailableProviders = function () {
    var _this = this;
    return Array.from(this.providers.keys()).filter(function (key) {
      var _a;
      return (_a = _this.providers.get(key)) === null || _a === void 0 ? void 0 : _a.isAvailable;
    });
  };
  /**
              ...
              return __generator(this, function (_c) {
                switch (_c.label) {
                  ...
                  case 2:
                    ...
                    lastError = error_1;
                    console.warn(
                      '[AI Provider] Attempt '.concat(attempt, '/').concat(maxRetries, ' failed:'),
                      error_1
                    );
                    ...
                    return [
                      ...
                      new Promise(function (resolve) {
                        return setTimeout(resolve, delay * Math.pow(2, attempt - 1));
                      }),
                    ];
                  case 3:
                    // Exponential backoff delay
...
export var validateAIProviders = function () {
  ...
  if (missing.length > 0) {
    console.error(
      '[AI Provider] Missing required environment variables: '.concat(missing.join(', '))
    );
    return false;
  }
