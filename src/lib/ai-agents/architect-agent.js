...
var ItineraryArchitectAgent = /** @class */ (function () {
      ...
      return __generator(this, function (_b) {
        switch (_b.label) {
          ...
          case 1:
            ...
            console.log(
              '\uD83D\uDD04 [84] Architect Agent: Generating architecture for workflow '.concat(
                input.workflowId.substring(0, 15),
                '...'
              )
            );
            return [
          ...
          case 2:
            ...
            return [
              2 /*return*/,
              __assign(
                __assign(__assign({}, architecture), { processingTime: processingTime }),
          ...
          case 3:
            error_1 = _b.sent();
            console.error(
              '[Architect Agent] Failed for workflow '.concat(input.workflowId, ':'),
              error_1
            );
            throw new Error(
              'Architecture generation failed: '.concat(
  ...
  ItineraryArchitectAgent.prototype.buildUserPrompt = function (formData) {
    ...
    return 'Plan the architectural framework for this trip:\n\nDESTINATION & TIMING:\n- Location: '
      .concat(formData.location, '\n- Departure: ')
      .concat(formData.departDate, '\n- Return: ')
      .concat(formData.returnDate || 'Open-ended', '\n- Duration: ')
      .concat(totalDays, ' days\n- Group: ')
      .concat(formData.adults, ' adult(s)')
      .concat(
        formData.children > 0 ? ', '.concat(formData.children, ' child(ren)') : '',
        '\n\nBUDGET INFORMATION:\n- Total Budget: '
      )
      .concat(formData.budget.total, ' ')
      .concat(formData.budget.currency, '\n- Accommodation: ')
      .concat(formData.budget.breakdown.accommodation, ' ')
      .concat(formData.budget.currency, '\n- Food: ')
      .concat(formData.budget.breakdown.food, ' ')
      .concat(formData.budget.currency, '  \n- Activities: ')
      .concat(formData.budget.breakdown.activities, ' ')
      .concat(formData.budget.currency, '\n- Transportation: ')
      .concat(formData.budget.breakdown.transportation, ' ')
      .concat(formData.budget.currency, '\n- Flexibility: ')
      .concat(formData.budget.flexibility, '\n\nTRAVEL STYLE & PREFERENCES:\n- Pace: ')
      .concat(formData.travelStyle.pace, '\n- Accommodation Type: ')
      .concat(formData.travelStyle.accommodationType, '\n- Dining Preferences: ')
      .concat(formData.travelStyle.diningPreferences, '\n- Activity Level: ')
      .concat(formData.travelStyle.activityLevel, '\n- Cultural Immersion: ')
      .concat(
        formData.travelStyle.culturalImmersion,
        '\n\nINTERESTS & ACTIVITIES:\n- Primary Interests: '
      )
      .concat(formData.interests.join(', '), '\n- Things to Avoid: ')
      .concat(formData.avoidances.join(', ') || 'None specified', '\n- Dietary Restrictions: ')
      .concat(formData.dietaryRestrictions.join(', ') || 'None', '\n- Trip Vibe: ')
      .concat(formData.tripVibe, '\n- Experience Level: ')
      .concat(formData.travelExperience, '\n- Dinner Preference: ')
      .concat(formData.dinnerChoice, '\n\nADDITIONAL SERVICES:\n')
      .concat(
  ...
  ItineraryArchitectAgent.prototype.createFallbackArchitecture = function (formData) {
    ...
    var dailyBudgetBreakdown = Array.from({ length: totalDays }, function (_, i) {
      return {
        day: i + 1,
    ...
    var travelPhases = [
      ...
      {
        phase: 'exploration',
        days: Array.from({ length: Math.max(1, totalDays - 2) }, function (_, i) {
          return i + 2;
        }),
    ...
    return {
      ...
      planningContext: {
        tripStyle: ''
          .concat(formData.travelStyle.pace, ' pace with ')
          .concat(formData.travelStyle.culturalImmersion, ' cultural immersion'),
