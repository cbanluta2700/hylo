...
var InformationSpecialistAgent = /** @class */ (function () {
    ...
    InformationSpecialistAgent.prototype.processRecommendations = function (input) {
        return __awaiter(this, void 0, void 0, function () {
            var startTime, client, systemPrompt, userPrompt, result, processingTime, processedInfo, error_1;
            var _a;
            return __generator(this, function (_b) {
                switch (_b.label) {
                    ...
                    case 1:
                        ...
                        console.log("[Specialist Agent] Processing recommendations for workflow ".concat(input.workflowId));
                    ...
                    case 2:
                        ...
                        return [2 /*return*/, __assign(__assign(__assign({}, processedInfo), { processingTime: processingTime }), (((_a = result.usage) === null || _a === void 0 ? void 0 : _a.totalTokens) && { tokensUsed: result.usage.totalTokens }))];
                    case 3:
                        error_1 = _b.sent();
                        console.error("[Specialist Agent] Failed for workflow ".concat(input.workflowId, ":"), error_1);
                        throw new Error("Recommendation processing failed: ".concat(error_1 instanceof Error ? error_1.message : 'Unknown error'));
    ...
    InformationSpecialistAgent.prototype.buildUserPrompt = function (input) {
        return "Analyze and rank the gathered travel recommendations based on these specific traveler preferences:\n\nUSER PREFERENCES:\n- Primary Interests: ".concat(input.userPreferences.interests.join(', '), "\n- Things to Avoid: ").concat(input.userPreferences.avoidances.join(', ') || 'None specified', "\n- Travel Experience: ").concat(input.userPreferences.travelExperience, "\n- Trip Vibe: ").concat(input.userPreferences.tripVibe, "\n\nTRIP ARCHITECTURE:\n- Total Days: ").concat(input.architecture.itineraryStructure.totalDays, "\n- Trip Style: ").concat(input.architecture.planningContext.tripStyle, "\n- Budget Strategy: ").concat(input.architecture.planningContext.budgetStrategy, "\n- Experience Goals: ").concat(input.architecture.planningContext.experienceGoals.join(', '), "\n\nTRAVEL PHASES:\n").concat(input.architecture.itineraryStructure.travelPhases
            .map(function (phase, i) { var _a; return "Phase ".concat(i + 1, ": ").concat(phase.focus, " (Days ").concat(((_a = phase.days) === null || _a === void 0 ? void 0 : _a.join(', ')) || 'N/A', ")"); })
            .join('\n'), "\n\nGATHERED ACCOMMODATIONS:\n").concat(input.gatheredInfo.accommodations
            .map(function (acc, i) {
            return "".concat(i + 1, ". ").concat(acc.name, " (").concat(acc.type, ", ").concat(acc.priceRange, ", Rating: ").concat(acc.rating, ") - ").concat(acc.location);
        })
            .join('\n'), "\n\nGATHERED RESTAURANTS:\n").concat(input.gatheredInfo.restaurants
            .map(function (rest, i) {
            return "".concat(i + 1, ". ").concat(rest.name, " (").concat(rest.cuisine, ", ").concat(rest.priceRange, ", Rating: ").concat(rest.rating, ") - ").concat(rest.location);
        })
            .join('\n'), "\n\nGATHERED ACTIVITIES:\n").concat(input.gatheredInfo.activities
            .map(function (act, i) {
            return "".concat(i + 1, ". ").concat(act.name, " (").concat(act.type, ", ").concat(act.cost, ", Rating: ").concat(act.rating, ") - ").concat(act.description);
        })
    ...
    InformationSpecialistAgent.prototype.createFallbackRanking = function (input) {
        var interests = input.userPreferences.interests.map(function (i) { return i.toLowerCase(); });
        return {
            rankedRecommendations: {
                accommodations: input.gatheredInfo.accommodations.map(function (acc, i) { return ({
                    id: "acc_".concat(i),
                    name: acc.name,
                ...
                restaurants: input.gatheredInfo.restaurants.map(function (rest, i) { return ({
                    id: "rest_".concat(i),
                    name: rest.name,
                ...
                activities: input.gatheredInfo.activities.map(function (act, i) {
                    var matchScore = interests.some(function (interest) {
                        return act.name.toLowerCase().includes(interest) ||
                    ...
                    return {
                        ...
                        recommendedDay: Math.floor(Math.random() * input.architecture.itineraryStructure.totalDays) + 1,
                    };
                }),
            },
            filteredOptions: {
